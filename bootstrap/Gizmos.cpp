	...
	m_2Dtris(new GizmoTri[max2DTris]) 
{
	...
	glShaderSource(vs, 1, &sk_gizmoVertexShader, 0);
	glCompileShader(vs);

	glShaderSource(fs, 1, &sk_gizmoFragmentShader, 0);
	glCompileShader(fs);
...
void Gizmos::AddAABB(const glm::vec3& center, const glm::vec3& rvExtents, const glm::vec4& colour, const glm::mat4* transform) 
{
	glm::vec3 vVerts[8];
	glm::vec3 c = center;
...
{
	if (sm_singleton && sm_singleton->m_lineCount < sm_singleton->m_maxLines) 
	{
...
{
	if (sm_singleton && sm_singleton->m_2DlineCount < sm_singleton->m_max2DLines) 
	{
...
{
	if (sm_singleton && (sm_singleton->m_lineCount > 0 || sm_singleton->m_triCount > 0 || sm_singleton->m_transparentTriCount > 0)) 
	{
		int shader = 0;
...
{
	if (sm_singleton && (sm_singleton->m_2DlineCount > 0 || sm_singleton->m_2DtriCount > 0)) 
	{
		int shader = 0;
		glGetIntegerv(GL_CURRENT_PROGRAM, &shader);
